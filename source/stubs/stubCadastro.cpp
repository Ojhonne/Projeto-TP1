...
// Inicializa o vetor est�tico limpo
...
bool StubCadastro::atualizarPessoa(const Pessoa& pessoa) {
    ...
    // grava o registro de Log no arquivo f�sico TXT
    std::ofstream arquivo("banco_simulado.txt", std::ios::app);
    if (arquivo.is_open()) {
    ...
    // atualiza os dados no vetor em mem�ria RAM
    for (size_t i = 0; i < pessoasCadastradas.size(); i++) {
    ...
    // se o vetor estava vazio (teste do joao), insere ele para as pr�ximas consultas
    pessoasCadastradas.push_back(pessoa);
    return true;
}
...
bool StubCadastro::excluirPessoa(const Email& email) {
    ...
    return true; // Retorna true para o login fict�cio funcionar mesmo com o vetor vazio
}
*/